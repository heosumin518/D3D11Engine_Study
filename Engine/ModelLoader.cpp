...
Engine::ModelLoader::ModelLoader(ComPtr<ID3D11Device> device)
	: m_device(device)
{
...
shared_ptr<Engine::Model> Engine::ModelLoader::LoadModelFile(const string& file)
{
...
void Engine::ModelLoader::CreateNode(shared_ptr<Model> model, aiNode* srcNode, shared_ptr<Node> parent)
{
...
void Engine::ModelLoader::CreateMesh(aiNode* node, shared_ptr<Node> connectNode)
{
...
void Engine::ModelLoader::CreateMaterial()
{
	...
	for (UINT i = 0; i < m_scene->mNumMaterials; i++)
	{
		...
		if (AI_SUCCESS == srcMaterial->GetTexture(aiTextureType_DIFFUSE, 0, &texturePath))
		{
			...
			CHECK(CreateTextureFromFile(finalPath.c_str(), material->m_diffuseRV.GetAddressOf()));
		}
		...
		if (AI_SUCCESS == srcMaterial->GetTexture(aiTextureType_NORMALS, 0, &texturePath))
		{
			path = ToWString(string(texturePath.C_Str()));
			finalPath = basePath + path.filename().wstring();
			CHECK(CreateTextureFromFile(finalPath.c_str(), material->m_normalRV.GetAddressOf()));
		}
		...
		if (AI_SUCCESS == srcMaterial->GetTexture(aiTextureType_SPECULAR, 0, &texturePath))
		{
			path = ToWString(string(texturePath.C_Str()));
			finalPath = basePath + path.filename().wstring();
			CHECK(CreateTextureFromFile(finalPath.c_str(), material->m_specularRV.GetAddressOf()));
		}
		...
		if (AI_SUCCESS == srcMaterial->GetTexture(aiTextureType_EMISSIVE, 0, &texturePath))
		{
			path = ToWString(string(texturePath.C_Str()));
			finalPath = basePath + path.filename().wstring();
			CHECK(CreateTextureFromFile(finalPath.c_str(), material->m_emissiveRV.GetAddressOf()));
		}
		...
		if (AI_SUCCESS == srcMaterial->GetTexture(aiTextureType_OPACITY, 0, &texturePath))
		{
			path = ToWString(string(texturePath.C_Str()));
			finalPath = basePath + path.filename().wstring();
			CHECK(CreateTextureFromFile(finalPath.c_str(), material->m_opacityRV.GetAddressOf()));
		}
		...
		if (AI_SUCCESS == srcMaterial->GetTexture(aiTextureType_METALNESS, 0, &texturePath))
		{
			path = ToWString(string(texturePath.C_Str()));
			finalPath = basePath + path.filename().wstring();
			CHECK(CreateTextureFromFile(finalPath.c_str(), material->m_metalnessRV.GetAddressOf()));
		}
		...
		if (AI_SUCCESS == srcMaterial->GetTexture(aiTextureType_SHININESS, 0, &texturePath))
		{
			path = ToWString(string(texturePath.C_Str()));
			finalPath = basePath + path.filename().wstring();
			CHECK(CreateTextureFromFile(finalPath.c_str(), material->m_roughnessRV.GetAddressOf()));
		}
...
void Engine::ModelLoader::CreateAnimation(aiAnimation* srcAnim)
{
...
shared_ptr<Engine::NodeAnimation> Engine::ModelLoader::ParseAnimationNode(shared_ptr<Animation> animation, aiNodeAnim* srcNodeAnim)
{
